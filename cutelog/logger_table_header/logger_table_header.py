...
class Column:
    ...
    def __repr__(self):
        return "{}(name={}, title={}, width={})".format(
            self.__class__.__name__, self.name, self.title, self.width
        )
...
class LoggerTableHeader(QObject):
    ...
    def regen_visible(self):
        ...
        self.visible_names = (
            set([c.name for c in self.visible_columns]) | SPECIAL_COLUMNS
        )
...
class HeaderEditDialog(QDialog):
    ...
    def accept(self):
        ...
        self.header_changed.emit(
            self.preset_name, self.setAsDefaultCheckbox.isChecked(), self.columns
        )
        self.done(0)
