...
class LogServer(QTcpServer):
    ...
    def start(self):
        ...
        if not result:
            ...
            show_critical_dialog(
                self.main_window, "Error while starting the server", err_string
            )
        else:
    ...
    def wait_connections_stopped(self):
        self.log.debug(
            "Waiting for {} connections threads to stop".format(len(self.threads))
        )
        for thread in self.threads.copy():
            try:
                if not thread.wait(1500):
                    ...
                    self.log.debug(
                        'Thread "{}" didn\'t stop in time, exiting'.format(thread)
                    )
                    return
    ...
    def cleanup_connection(self, connection):
        try:
            ...
            self.log.error(
                "Double delete on connection: {}".format(connection), exc_info=True
            )
            return
        ...
        def wait_and_read(n_bytes):
            ...
            while len(data) < n_bytes:
                if sock.bytesAvailable() == 0:
                    new_data = sock.waitForReadyRead(
                        100
                    )  # wait for 100ms between read attempts
                    if not new_data:
    ...
    def handle_internal_command(self, data):
        ...
        if cmd == "format":
            if value in self.serializers:
                ...
                self.log.error(
                    'Serialization format "{}" is not supported'.format(value)
                )
        else:
...
class BenchmarkMonitor(QThread):
    ...
    def run(self):
        ...
        while True:
            ...
            status = "{} rows/s, average: {} rows/s".format(
                self.logger.monitor_count * 2, average
            )
