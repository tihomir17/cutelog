...
class Config(QObject):
    ...
    def listen_address(self):
        ...
        if host is None or port is None:
            raise Exception(
                'Listen host or port not in options: "{}:{}"'.format(host, port)
            )
        return (host, port)
    ...
    def load_options(self):
        ...
        for option in self.option_spec:
            ...
            if option.type == bool:
                value = str(
                    value
                ).lower()  # needed because QSettings stores bools as strings
                ...
                try:
                    ...
                    self.log.warn(
                        'Could not parse value "{}" for option "{}", falling back to the '
    ...
    def update_attributes(self, options=None):
        ...
        self.benchmark_interval = options.get(
            "benchmark_interval", self.benchmark_interval
        )
        self.logger_table_font = options.get(
            "logger_table_font", self.logger_table_font
        )
        self.logger_table_font_size = options.get(
            "logger_table_font_size", self.logger_table_font_size
        )
        self.logger_row_height = options.get(
            "logger_row_height", self.logger_row_height
        )
